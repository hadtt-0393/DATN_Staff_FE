...
export default function ProfileView() {
    ...
    useEffect(() => {
        ...
        const getProfile = async () => {
            ...
            const serviceHotel = await axiosInstance.get(
                "/serviceHotel/getAllServiceHotelSystem"
            );
            setHotel(res.data);
    ...
    const handleChangeHotel = (values: any) => {
        setHotel({
            ...hotel,
            ...values,
        });
    ...
    const uploadImg = async (file: File) => {
        ...
        return axios
            .post(REACT_APP_CLOUDINARY_ENDPOINT, formData)
            .then((response) => {
    ...
    const save = async () => {
        ...
        const saveProfile = async () => {
            let hotelUpdate: any;
            if (files) {
                let uploaders = Array.from(files).map(uploadImg);
                let data = await Promise.all(uploaders);
                hotelUpdate = { ...hotel, images: data };
            } else {
                hotelUpdate = hotel;
            }
            const res = await axiosInstance.put(
                "/hotel/update-detail-hotel",
                hotelUpdate
            );
            setHotel(res.data);
            setLoading(false);
